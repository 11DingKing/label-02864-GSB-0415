<template>
  <div class="page-container">
      ...
      <div v-for="(q, index) in questions" :key="q.id" class="question-item">
        <div class="question-header">
          ...
          <div class="question-actions" v-if="exam?.status === 0">
            <el-button type="primary" link size="small" @click="openDialog(q)">编辑</el-button>
            <el-button type="danger" link size="small" @click="handleDelete(q)">删除</el-button>
          </div>
        </div>
        <div class="question-content">{{ q.content }}</div>
        <div class="question-options">
          <div v-for="opt in q.options" :key="opt.id" :class="['option-item', { correct: opt.isCorrect }]">
    ...
    <el-dialog v-model="dialogVisible" :title="editingQuestion ? '编辑题目' : '添加题目'" width="600px">
        ...
        <el-form-item label="选项">
          <div class="options-editor">
            <div v-for="(opt, idx) in form.options" :key="idx" class="option-row">
              <el-checkbox v-model="opt.isCorrectBool" :disabled="form.type === 1 && hasOtherCorrect(idx)">
                正确
              </el-checkbox>
            ...
            <el-button v-if="form.type !== 3 && form.options.length < 6" type="primary" link @click="addOption">
...
import { getQuestionsByExamId, createQuestion, updateQuestion, deleteQuestion } from '../../api/question'
...
const defaultOptions = () => [
  { optionKey: 'A', content: '', isCorrect: 0, isCorrectBool: false, sortOrder: 0 },
  { optionKey: 'B', content: '', isCorrect: 0, isCorrectBool: false, sortOrder: 1 },
  { optionKey: 'C', content: '', isCorrect: 0, isCorrectBool: false, sortOrder: 2 },
  { optionKey: 'D', content: '', isCorrect: 0, isCorrectBool: false, sortOrder: 3 }
]

const judgmentOptions = () => [
  { optionKey: 'A', content: '正确', isCorrect: 0, isCorrectBool: false, sortOrder: 0 },
  { optionKey: 'B', content: '错误', isCorrect: 0, isCorrectBool: false, sortOrder: 1 }
]
...
watch(() => form.type, (val) => {
  ...
  } else if (form.options.length === 2 && form.options[0].content === '正确') {
...
const openDialog = (q = null) => {
  ...
  if (q) {
    ...
    form.score = q.score
    form.options = q.options.map(o => ({ ...o, isCorrectBool: o.isCorrect === 1 }))
  } else {
    form.type = 1
    form.content = ''
    form.score = 10
...
const handleDelete = async (q) => {
  await ElMessageBox.confirm('确定要删除此题目吗？', '提示', { type: 'warning' })
