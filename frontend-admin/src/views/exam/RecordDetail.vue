<template>
  <div class="page-container">
    <div class="page-header">
      ...
      <div>
        <el-tag :type="getStatusType(record)" size="large">{{ getStatusText(record) }}</el-tag>
        <span class="total-score">总分: {{ record?.totalScore }}分</span>
        <el-button @click="$router.back()" style="margin-left: 16px">返回</el-button>
      </div>
    </div>

    <div class="card" v-for="(item, index) in questionWithAnswers" :key="item.question.id" class="question-card">
      <div class="question-header">
        <span class="question-num">{{ index + 1 }}.</span>
        <el-tag size="small" :type="getTypeTag(item.question.type)">{{ getTypeText(item.question.type) }}</el-tag>
      ...
      <div class="question-options" v-if="item.question.type !== 4">
        <div v-for="opt in item.question.options" :key="opt.id" :class="['option-item', { correct: opt.isCorrect, selected: isSelected(opt, item.answer) }]">
          <span class="option-key">{{ opt.optionKey }}.</span>
          <span>{{ opt.content }}</span>
          <el-icon v-if="opt.isCorrect" class="correct-icon"><Check /></el-icon>
          <el-icon v-if="isSelected(opt, item.answer) && !opt.isCorrect" class="wrong-icon"><Close /></el-icon>
        </div>
      </div>
      ...
      <div v-if="item.question.type === 4" class="essay-container">
        ...
        <div class="reference-section">
          ...
          <div class="essay-content">{{ item.question.referenceAnswer || '无' }}</div>
        </div>
        <div class="keywords-section">
          <div class="section-title">关键词:</div>
          <div class="keywords-list">
            <el-tag v-for="kw in item.question.keywords?.split(',')" :key="kw" size="small" type="info">{{ kw.trim() }}</el-tag>
...
import { getRecordDetail, getRecordAnswers, updateAnswerScore } from '../../api/record'
...
const getTypeText = (type) => ({ 1: '单选', 2: '多选', 3: '判断', 4: '简答' }[type])
const getTypeTag = (type) => ({ 1: '', 2: 'warning', 3: 'info', 4: 'success' }[type])
