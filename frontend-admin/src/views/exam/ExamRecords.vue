      ...
      <el-table :data="records" v-loading="loading" stripe style="width: 100%">
        ...
        <el-table-column label="得分" min-width="100" align="center">
          <template #default="{ row }">
            <span :class="row.totalScore >= (exam?.passScore || 60) ? 'score-pass' : 'score-fail'">
              {{ row.totalScore }}
            </span>
          </template>
        </el-table-column>
        <el-table-column label="是否及格" min-width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="row.totalScore >= (exam?.passScore || 60) ? 'success' : 'danger'">
...
const stats = ref({ total: 0, submittedCount: 0, passRate: 0, avgScore: 0, maxScore: 0 })
...
const loadRecords = async () => {
  loading.value = true
  try {
    const res = await getExamRecords(examId, { page: pagination.page, size: pagination.size })
...
const getStatusText = (row) => {
  ...
  if (row.startTime && exam.value?.duration) {
    ...
    const deadline = new Date(startTime.getTime() + exam.value.duration * 60 * 1000)
